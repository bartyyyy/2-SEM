...
int menu(void)
{
	...
	cout << "7. Добавление числа в начало списка" << endl;
	cout << "8. Выход" << endl;
	cout << endl;
	do
	{
...
void addXBegin(int x, Address** phead, Address** plast) //Добавление в начало списка числа x
{
	...
	if (*phead != nullptr) {
		// Устанавливаем указатель prev у текущего первого элемента на новый элемент
		(*phead)->prev = e;
	}
	else {	
		// Если список пустой, устанавливаем указатель plast на новый элемент
		*plast = e;
	}

	// Обновляем указатель phead на новый элемент
	*phead = e;

	cout << "Число добавлено в начало списка" << endl;
}
